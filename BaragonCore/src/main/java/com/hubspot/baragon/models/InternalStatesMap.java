...
public class InternalStatesMap {
  private static EnumMap<InternalRequestStates, AgentRequestType> stateToRequestTypeMap = new EnumMap<>(
    InternalRequestStates.class
  );

  static {
    stateToRequestTypeMap.put(InternalRequestStates.PENDING, AgentRequestType.APPLY);
    stateToRequestTypeMap.put(
      InternalRequestStates.INVALID_REQUEST_NOOP,
    ...
    stateToRequestTypeMap.put(
      InternalRequestStates.CHECK_APPLY_RESPONSES,
      AgentRequestType.APPLY
    );
    stateToRequestTypeMap.put(InternalRequestStates.COMPLETED, AgentRequestType.APPLY);
    stateToRequestTypeMap.put(
      InternalRequestStates.FAILED_SEND_REVERT_REQUESTS,
      AgentRequestType.REVERT
    );
    stateToRequestTypeMap.put(
      InternalRequestStates.FAILED_CHECK_REVERT_RESPONSES,
      AgentRequestType.REVERT
    );
    stateToRequestTypeMap.put(
      InternalRequestStates.FAILED_REVERT_FAILED,
      AgentRequestType.REVERT
    );
    stateToRequestTypeMap.put(
      InternalRequestStates.CANCELLED_SEND_REVERT_REQUESTS,
      AgentRequestType.CANCEL
    );
    stateToRequestTypeMap.put(
      InternalRequestStates.CANCELLED_CHECK_REVERT_RESPONSES,
      AgentRequestType.CANCEL
    );
    stateToRequestTypeMap.put(InternalRequestStates.CANCELLED, AgentRequestType.CANCEL);
    stateToRequestTypeMap.put(
      InternalRequestStates.FAILED_CANCEL_FAILED,
      AgentRequestType.CANCEL
    );
  }

  private static EnumMap<InternalRequestStates, BaragonRequestState> stateToRequestStateMap = new EnumMap<>(
    InternalRequestStates.class
  );

  static {
    stateToRequestStateMap.put(
    ...
    stateToRequestStateMap.put(
      InternalRequestStates.COMPLETED_POST_APPLY_FAILED,
      BaragonRequestState.SUCCESS
    );
  }

  private static EnumMap<AgentRequestType, EnumMap<AgentRequestsStatus, InternalRequestStates>> typeToRequestStateMap = new EnumMap<>(
    AgentRequestType.class
  );

  static {
    EnumMap<AgentRequestsStatus, InternalRequestStates> applyMap = new EnumMap<>(
      AgentRequestsStatus.class
    );
    applyMap.put(
      AgentRequestsStatus.WAITING,
      InternalRequestStates.CHECK_APPLY_RESPONSES
    );
    applyMap.put(
    ...
    EnumMap<AgentRequestsStatus, InternalRequestStates> revertMap = new EnumMap<>(
      AgentRequestsStatus.class
    );
    revertMap.put(
      AgentRequestsStatus.WAITING,
    ...
    EnumMap<AgentRequestsStatus, InternalRequestStates> cancelMap = new EnumMap<>(
      AgentRequestsStatus.class
    );
  ...
  public static InternalRequestStates getWaitingState(
    InternalRequestStates requestState
  ) {
    return typeToRequestStateMap
      .get(stateToRequestTypeMap.get(requestState))
      .get(AgentRequestsStatus.WAITING);
  }

  public static InternalRequestStates getFailureState(
    InternalRequestStates requestState
  ) {
    return typeToRequestStateMap
      .get(stateToRequestTypeMap.get(requestState))
      .get(AgentRequestsStatus.FAILURE);
  }

  public static InternalRequestStates getRetryState(InternalRequestStates requestState) {
    return typeToRequestStateMap
      .get(stateToRequestTypeMap.get(requestState))
      .get(AgentRequestsStatus.RETRY);
  }

  public static InternalRequestStates getSuccessState(
    InternalRequestStates requestState
  ) {
    return typeToRequestStateMap
      .get(stateToRequestTypeMap.get(requestState))
      .get(AgentRequestsStatus.SUCCESS);
  }
  ...
  public static boolean isRemovable(InternalRequestStates requestState) {
    ...
    return (
      state == BaragonRequestState.SUCCESS ||
      state == BaragonRequestState.FAILED ||
      state == BaragonRequestState.CANCELED ||
      state == BaragonRequestState.INVALID_REQUEST_NOOP
    );
  }
}
