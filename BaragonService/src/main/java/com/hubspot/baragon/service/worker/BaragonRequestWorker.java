...
public class BaragonRequestWorker implements Runnable {
  ...
  private String buildResponseString(
    Map<String, Collection<AgentResponse>> agentResponses,
    AgentRequestType requestType
  ) {
    if (
      agentResponses.containsKey(requestType.name()) &&
      !agentResponses.get(requestType.name()).isEmpty()
    ) {
      Set<String> messages = new HashSet<>();
      for (AgentResponse response : agentResponses.get(requestType.name())) {
        if (
          response.toRequestStatus() == AgentRequestsStatus.FAILURE ||
          response.toRequestStatus() == AgentRequestsStatus.INVALID_REQUEST_NOOP
        ) {
          messages.add(
            String.format(
              "(%s) - %s",
              response.getStatusCode().or(0),
              response.getContent().or(response.getException()).or("")
            )
          );
        } else {
          messages.add(
            String.format("%s - %s", response.getUrl(), response.toRequestStatus().name())
          );
        }
      }
  ...
  private InternalRequestStates handleCheckRevertResponse(
    BaragonRequest request,
    InternalRequestStates currentState
  ) {
    final Map<String, Collection<AgentResponse>> agentResponses;

    switch (
      agentManager.getRequestsStatus(
        request,
        InternalStatesMap.getRequestType(currentState)
      )
    ) {
      case FAILURE:
        agentResponses =
          agentManager.getAgentResponses(request.getLoadBalancerRequestId());
        requestManager.setRequestMessage(
          request.getLoadBalancerRequestId(),
          String.format(
            "Apply failed {%s}, %s failed {%s}",
            buildResponseString(agentResponses, AgentRequestType.APPLY),
            InternalStatesMap.getRequestType(currentState).name(),
            buildResponseString(
              agentResponses,
              InternalStatesMap.getRequestType(currentState)
            )
  ...
  private InternalRequestStates handleState(
    InternalRequestStates currentState,
    BaragonRequest request
  ) {
    switch (currentState) {
      case PENDING:
        final Map<String, String> conflicts = requestManager.getBasePathConflicts(
          request
        );

        if (
          !conflicts.isEmpty() &&
          !(request.getAction().or(RequestAction.UPDATE) == RequestAction.DELETE)
        ) {
          requestManager.setRequestMessage(
            request.getLoadBalancerRequestId(),
            getBasePathConflictMessage(conflicts)
          );
          return InternalRequestStates.INVALID_REQUEST_NOOP;
        }

        final Set<String> missingGroups = requestManager.getMissingLoadBalancerGroups(
          request
        );
        ...
        if (!missingGroups.isEmpty()) {
          requestManager.setRequestMessage(
            request.getLoadBalancerRequestId(),
            String.format(
              "Invalid request due to non-existent load balancer groups: %s",
              missingGroups
            )
          );
          return InternalRequestStates.INVALID_REQUEST_NOOP;
        ...
        for (String loadBalancerGroup : request
          ...
          if (agentManager.invalidAgentCount(loadBalancerGroup)) {
            requestManager.setRequestMessage(
              request.getLoadBalancerRequestId(),
              String.format(
                "Invalid request due to not enough agents present for group: %s",
                loadBalancerGroup
              )
            );
        ...
        if (!request.getLoadBalancerService().getDomains().isEmpty()) {
          List<String> domainsNotServed = getDomainsNotServed(
            request.getLoadBalancerService()
          );
          if (!domainsNotServed.isEmpty()) {
            requestManager.setRequestMessage(
              request.getLoadBalancerRequestId(),
              String.format("No groups present that serve domains: %s", domainsNotServed)
            );
            return InternalRequestStates.INVALID_REQUEST_NOOP;
          }
        }
      ...
      case CHECK_APPLY_RESPONSES:
        switch (
          ...
          case FAILURE:
            final Map<String, Collection<AgentResponse>> agentResponses = agentManager.getAgentResponses(
              request.getLoadBalancerRequestId()
            );
            requestManager.setRequestMessage(
              request.getLoadBalancerRequestId(),
              String.format(
                "Apply failed (%s), reverting...",
                buildResponseString(
                  agentResponses,
                  InternalStatesMap.getRequestType(currentState)
                )
          ...
          case SUCCESS:
            try {
              requestManager.setRequestMessage(
                request.getLoadBalancerRequestId(),
                String.format(
                  "%s request succeeded! Added upstreams: %s, Removed upstreams: %s",
                  request.getAction().or(RequestAction.UPDATE),
                  request.getAddUpstreams(),
                  request.getRemoveUpstreams()
                )
              );
              ...
              String message = String.format(
                "Caught zookeeper error for path %s.",
                ke.getPath()
              );
              ...
              LOG.warn(
                String.format(
                  "Request %s was successful, but failed to commit!",
                  request.getLoadBalancerRequestId()
                ),
                e
              );
      ...
      case CANCELLED_SEND_REVERT_REQUESTS:
        throw new RuntimeException(
          String.format(
            "Requests in state %s must be handled by batch request sender",
            currentState
          )
        );
  ...
  private String getBasePathConflictMessage(Map<String, String> conflicts) {
    ...
    for (Map.Entry<String, String> entry : conflicts.entrySet()) {
      message =
        String.format("%s %s on group %s,", message, entry.getValue(), entry.getKey());
    }
  ...
  private Map<QueuedRequestWithState, InternalRequestStates> handleQueuedRequests(
    ...
    for (QueuedRequestWithState queuedRequestWithState : queuedRequestsWithState) {
      ...
      if (!queuedRequestWithState.getCurrentState().isRequireAgentRequest()) {
        try {
          results.put(
            queuedRequestWithState,
            handleState(
              queuedRequestWithState.getCurrentState(),
              queuedRequestWithState.getRequest()
            )
          );
        } catch (Exception e) {
          LOG.error(
            "Error processing request {}",
            queuedRequestWithState.getRequest().getLoadBalancerRequestId(),
            e
          );
        }
      } else {
  ...
  public void run() {
    ...
    try {
      final List<QueuedRequestWithState> queuedRequests = requestManager
        .getQueuedRequestIds()
        .stream()
      ...
      List<QueuedRequestWithState> inFlightRequests = queuedRequests
        .stream()
        .filter(q -> q.getCurrentState().isInFlight() || hasInProgressAttempt(q))
        .collect(Collectors.toList());

      final Set<String> inProgressServices = inFlightRequests
        .stream()
        .map(q -> q.getQueuedRequestId().getServiceId())
        .collect(Collectors.toSet());

      final Set<QueuedRequestWithState> removedForCurrentInFlightRequest = queuedRequests
        .stream()
        .filter(
          q ->
            inProgressServices.contains(q.getQueuedRequestId().getServiceId()) &&
            !inFlightRequests.contains(q)
        )
        .collect(Collectors.toSet());
      if (!inFlightRequests.isEmpty()) {
        LOG.info(
          "Skipping new updates for services {} due to current in-flight updates",
          String.join(",", inProgressServices)
        );
      }
      ...
      LOG.debug(
        "Processing {} BaragonRequests which are already in-flight",
      ...
      while (
        added < configuration.getWorkerConfiguration().getMaxRequestsPerPoll() &&
        ...
        List<QueuedRequestWithState> hydratedNonServiceChanges = nonServiceChanges
          .stream()
          .filter(
            q -> {
              ...
              if (inProgressServices.contains(q.getQueuedRequestId().getServiceId())) {
                LOG.info(
                  "Skipping {} because {} already has an in progress request",
                  q.getQueuedRequestId().getRequestId(),
                  q.getQueuedRequestId().getServiceId()
                );
                return false;
              }
              return true;
        ...
        // Then send them off.
        LOG.debug(
          "Processing {} BaragonRequests which don't modify a BaragonService",
          nonServiceChanges.size()
        );
        handleResultStates(handleQueuedRequests(hydratedNonServiceChanges));

        queuedRequests.removeAll(nonServiceChanges);
        inProgressServices.addAll(
          hydratedNonServiceChanges
            .stream()
            .map(q -> q.getQueuedRequestId().getServiceId())
            .collect(Collectors.toSet())
        );
        ...
        List<QueuedRequestWithState> hydratedServiceChanges = serviceChanges
          .stream()
          .filter(
            q -> {
              if (inProgressServices.contains(q.getQueuedRequestId().getServiceId())) {
        ...
        LOG.debug(
          "Processing {} BaragonRequests which modify a BaragonService",
          serviceChanges.size()
        );
  ...
  private QueuedRequestWithState saveAdjustedRequest(
    MaybeAdjustedRequest maybeAdjustedRequest
  ) {
    if (maybeAdjustedRequest.wasAdjusted) {
      try {
        ...
        LOG.warn(
          "Unable to save adjustments for request {}",
          maybeAdjustedRequest.request.getQueuedRequestId().getRequestId(),
          e
        );
      }
    }
  ...
  private MaybeAdjustedRequest setNoValidateIfRequestRemovesUpstreamsOnly(
    MaybeAdjustedRequest nonServiceChangeRequest
  ) {
    ...
    boolean upstreamRemovalsOnly =
      !originalRequest.getRemoveUpstreams().isEmpty() &&
      originalRequest.getAddUpstreams().isEmpty() &&
      originalRequest.getReplaceUpstreams().isEmpty();

    if (upstreamRemovalsOnly) {
      LOG.trace(
        "Request {} does not change a BaragonService and only removes upstreams. Setting noValidate.",
        nonServiceChangeRequest.request.getQueuedRequestId().getRequestId()
      );
      ...
      BaragonRequest requestWithNoValidate = originalRequest
        .toBuilder()
        .setNoValidate(true)
        .build();

      return new MaybeAdjustedRequest(
  ...
  private MaybeAdjustedRequest preResolveDNS(
    MaybeAdjustedRequest nonServiceChangeRequest
  ) {
    if (
      !nonServiceChangeRequest
        .request.getRequest()
        .getLoadBalancerService()
        .isPreResolveUpstreamDNS()
    ) {
      return nonServiceChangeRequest;
    }
    ...
    List<UpstreamInfo> maybeResolvedAddUpstreams = resolveDNSForAllUpstreams(
      nonServiceChangeRequest.request.getRequest().getAddUpstreams()
    );
    List<UpstreamInfo> maybeResolvedRemoveUpstreams = resolveDNSForAllUpstreams(
      nonServiceChangeRequest.request.getRequest().getRemoveUpstreams()
    );
    List<UpstreamInfo> maybeResolvedReplaceUpstreams = resolveDNSForAllUpstreams(
      nonServiceChangeRequest.request.getRequest().getReplaceUpstreams()
    );

    if (
      allUpstreamsAreResolved(maybeResolvedAddUpstreams) &&
      allUpstreamsAreResolved(maybeResolvedRemoveUpstreams) &&
      allUpstreamsAreResolved(maybeResolvedReplaceUpstreams)
    ) {
      LOG.trace(
        "Request {} does not change a BaragonService and all upstreams were pre-resolved. Setting noValidate.",
        nonServiceChangeRequest.request.getQueuedRequestId().getRequestId()
      );
      BaragonRequest requestWithResolvedUpstreams = originalRequest
        .toBuilder()
  ...
  private boolean allUpstreamsAreResolved(Collection<UpstreamInfo> upstreams) {
    ...
    return upstreams
      .stream()
      .allMatch(upstreamInfo -> upstreamInfo.getResolvedUpstream().isPresent());
  }

  private List<UpstreamInfo> resolveDNSForAllUpstreams(
      ...
      .map(
        upstreamInfo ->
          new UpstreamInfo(
            ...
            upstreamInfo.getRequestId(),
            upstreamInfo.getRackId(),
  ...
  private void handleResultStates(
    Map<QueuedRequestWithState, InternalRequestStates> results
  ) {
    for (Map.Entry<QueuedRequestWithState, InternalRequestStates> result : results.entrySet()) {
      if (result.getValue() != result.getKey().getCurrentState()) {
        LOG.info(
          String.format(
            "%s: %s --> %s",
            result.getKey().getQueuedRequestId().getRequestId(),
            result.getKey().getCurrentState(),
            result.getValue()
          )
        );
        requestManager.setRequestState(
          result.getKey().getQueuedRequestId().getRequestId(),
          result.getValue()
        );
      }
      ...
      if (InternalStatesMap.isRemovable(result.getValue())) {
        ...
        requestManager.saveResponseToHistory(
          result.getKey().getRequest(),
          result.getValue()
        );
  ...
  private void collectRequests(
    ...
    for (QueuedRequestWithState queuedRequest : queuedRequests) {
      ...
      if (
        requestManager
          .getRequest(queuedRequest.getQueuedRequestId().getRequestId())
          .transform(someRequest -> !isBatchBoundary(someRequest))
          .or(false)
      ) {
  ...
  private boolean isBatchBoundary(BaragonRequest baragonRequest) {
    return (
      !stateDatastore.isServiceUnchanged(baragonRequest) ||
      !baragonRequest.getAction().or(RequestAction.UPDATE).equals(RequestAction.UPDATE)
    );
  }

  private Optional<QueuedRequestWithState> hydrateQueuedRequestWithState(
    QueuedRequestId queuedRequestId
  ) {
    final String requestId = queuedRequestId.getRequestId();
    final Optional<InternalRequestStates> maybeState = requestManager.getRequestState(
      requestId
    );
    ...
    return Optional.of(
      new QueuedRequestWithState(queuedRequestId, maybeRequest.get(), maybeState.get())
    );
  }
  ...
  static Comparator<QueuedRequestWithState> queuedRequestComparator() {
    return (requestA, requestB) -> {
      ...
      if (
        (requestA.getRequest().isNoValidate() && requestA.getRequest().isNoReload()) &&
        (!requestB.getRequest().isNoReload() || !requestB.getRequest().isNoValidate())
      ) {
        return -1;
      }

      if (
        (requestB.getRequest().isNoValidate() && requestB.getRequest().isNoReload()) &&
        (!requestA.getRequest().isNoReload() || !requestA.getRequest().isNoValidate())
      ) {
        return 1;
      }
      ...
      return Integer.compare(
        requestA.getQueuedRequestId().getIndex(),
        requestB.getQueuedRequestId().getIndex()
      );
    };
  }
}
