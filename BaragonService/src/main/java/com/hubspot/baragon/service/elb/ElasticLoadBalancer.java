...
public abstract class ElasticLoadBalancer {
  ...
  public abstract AgentCheckInResponse removeInstance(
    Instance instance,
    String id,
    String elbName,
    String agentId
  ...
  public abstract AgentCheckInResponse checkRemovedInstance(
    String id,
    String elbName,
    String agentId
  );

  public abstract AgentCheckInResponse registerInstance(
    Instance instance,
    String id,
    String elbName,
  ...
  public abstract AgentCheckInResponse checkRegisteredInstance(
    Instance instance,
    String id,
  ...
  Optional<BaragonKnownAgentMetadata> knownAgent(BaragonGroup group, String instanceId) {
    Collection<BaragonKnownAgentMetadata> knownAgents = knownAgentsDatastore.getKnownAgentsMetadata(
      group.getName()
    );
    for (BaragonKnownAgentMetadata agent : knownAgents) {
      if (
        agent.getEc2().getInstanceId().isPresent() &&
        agent.getEc2().getInstanceId().get().equals(instanceId)
      ) {
    ...
    if (!agent.isPresent()) {
      ...
      if (configuration.get().isRemoveKnownAgentEnabled()) {
        ...
        Date threshold = new Date(
          ...
          (configuration.get().getRemoveKnownAgentMinutes() * 60000L)
        );
