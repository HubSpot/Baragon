...
public class AgentManager {
  ...
  private AsyncHttpClient.BoundRequestBuilder buildAgentBatchRequest(
    String url,
    List<BaragonRequestBatchItem> batch
  )
  ...
  public Map<QueuedRequestWithState, InternalRequestStates> sendRequests(
    final List<QueuedRequestWithState> queuedRequestsWithState
  ) {
    ...
    for (QueuedRequestWithState queuedRequestWithState : queuedRequestsWithState) {
      ...
      for (String group : loadBalancerGroupsToUpdate) {
        for (BaragonAgentMetadata agentMetadata : agentMetadataByGroup.computeIfAbsent(
          group,
          loadBalancerDatastore::getAgentMetadata
        )) {
          int inFlight = agentResponseDatastore.getPendingRequestsCount(
            agentMetadata.getBaseAgentUri()
          );
          if (inFlight >= configuration.getMaxConcurrentRequestsPerAgent()) {
            LOG.info(
              "Too many concurrent requests for group {} ({} >= {}), waiting for some to complete before attempting request {}",
              group,
              inFlight,
              configuration.getMaxConcurrentRequestsPerAgent(),
              queuedRequestWithState.getQueuedRequestId().getRequestId()
            );
      ...
      if (maybeOriginalService.isPresent()) {
        loadBalancerGroupsToUpdate.addAll(
          maybeOriginalService.get().getLoadBalancerGroups()
        );
      }
      for (String group : loadBalancerGroupsToUpdate) {
        if (requestsByGroup.containsKey(group)) {
          requestsByGroup
            .get(group)
            .add(
              new BaragonRequestBatchItem(
                request.getLoadBalancerRequestId(),
                request.getAction(),
                InternalStatesMap.getRequestType(queuedRequestWithState.getCurrentState())
              )
            );
        } else {
          List<BaragonRequestBatchItem> batchItemsForGroup = new ArrayList<>();
          batchItemsForGroup.add(
            new BaragonRequestBatchItem(
              request.getLoadBalancerRequestId(),
              request.getAction(),
              InternalStatesMap.getRequestType(queuedRequestWithState.getCurrentState())
      ...
      results.put(
        ...
        InternalStatesMap.getWaitingState(queuedRequestWithState.getCurrentState())
      );
    }

    for (Map.Entry<String, List<BaragonRequestBatchItem>> entry : requestsByGroup.entrySet()) {
      for (final BaragonAgentMetadata agentMetadata : loadBalancerDatastore.getAgentMetadata(
        entry.getKey()
      )) {
        ...
        if (agentMetadata.isBatchEnabled()) {
          ...
          for (BaragonRequestBatchItem batchItem : entry.getValue()) {
            sendIndividualRequest(
              baseUrl,
              batchItem.getRequestId(),
              batchItem.getRequestType()
            );
          }
        }
      }
  ...
  private void sendBatchRequest(
    ...
    final List<BaragonRequestBatchItem> originalBatch
  ) {
    ...
    for (BaragonRequestBatchItem item : batch) {
      if (!shouldSendRequest(baseUrl, item.getRequestId(), item.getRequestType())) {
        ...
        agentResponseDatastore.setPendingRequestStatus(
          item.getRequestId(),
          baseUrl,
          true
        );
  ...
  void sendFilteredBatchRequests(String baseUrl, List<BaragonRequestBatchItem> batch) {
    ...
    try {
      buildAgentBatchRequest(url, batch)
        .execute(
          new AsyncCompletionHandler<Void>() {
            ...
            public Void onCompleted(Response response) throws Exception {
              LOG.info(
                "Got HTTP {} from {} for batch request {}",
              ...
              if (response.getStatusCode() >= 300) {
                LOG.error(
                  "Received invalid response from agent (status: {}, response: {})",
                  response.getStatusCode(),
                ...
                for (BaragonRequestBatchItem item : batch) {
                  agentResponseDatastore.addAgentResponse(
                    ...
                    Optional.of(
                      String.format(
                        "Caught exception processing agent response %s",
                        response
                      )
                    )
                  );
              ...
              Set<AgentBatchResponseItem> responses = objectMapper.readValue(
                response.getResponseBody(),
                new TypeReference<Set<AgentBatchResponseItem>>() {}
              );
              for (AgentBatchResponseItem agentResponse : responses) {
                LOG.info("agentResponse={}", agentResponse);
                agentResponseDatastore.addAgentResponse(
                  agentResponse.getRequestId(),
                  agentResponse.getRequestType(),
                  baseUrl,
                  url,
                  Optional.of(agentResponse.getStatusCode()),
              ...
              for (BaragonRequestBatchItem item : batch) {
                ...
                if (!handledRequestIds.contains(item.getRequestId())) {
                  LOG.info(
                    "handledRequestIds doesn't contain item with id={}",
                    item.getRequestId()
                  );
                  agentResponseDatastore.addAgentResponse(
                    item.getRequestId(),
                    ...
                    Optional.of(
                      String.format(
                        "No response in batch for request %s",
                        item.getRequestId()
                      )
                    )
                  );
            ...
            public void onThrowable(Throwable t) {
              LOG.error(
                "Got exception when hitting {} with batch request {}",
                baseUrl,
                batch,
                t
              );
      ...
      LOG.info(
        "Got exception {} when hitting {} with batch reqeust {}",
        e,
        baseUrl,
        batch
      );
  ...
  private boolean shouldSendRequest(
    ...
    Optional<Long> maybePendingRequest = agentResponseDatastore.getPendingRequest(
      requestId,
      baseUrl
    );
    if (
      maybePendingRequest.isPresent() &&
      !(
        (System.currentTimeMillis() - maybePendingRequest.get()) >
        baragonAgentRequestTimeout
      )
    ) {
      LOG.info(
        String.format(
          "Request has been processing for %s ms",
          (System.currentTimeMillis() - maybePendingRequest.get())
        )
      );
      return false;
    ...
    final Optional<AgentResponseId> maybeLastResponseId = agentResponseDatastore.getLastAgentResponseId(
      requestId,
      requestType,
      baseUrl
    );
    ...
    if (
      maybeLastResponseId.isPresent() &&
      (
        maybeLastResponseId.get().getAttempt() > baragonAgentMaxAttempts ||
        maybeLastResponseId.get().isSuccess()
      )
    ) {
      return false;
    }
  ...
  void sendIndividualRequest(
    ...
    try {
      buildAgentRequest(url, requestType)
        .execute(
          new AsyncCompletionHandler<Void>() {
            ...
            public Void onCompleted(Response response) throws Exception {
              LOG.info(
                String.format(
                  "Got HTTP %d from %s for %s",
                  response.getStatusCode(),
                  baseUrl,
                  requestId
                )
              );
              final Optional<String> content = Strings.isNullOrEmpty(
                  response.getResponseBody()
                )
                ? Optional.<String>absent()
            ...
            public void onThrowable(Throwable t) {
              LOG.info(
                String.format(
                  "Got exception %s when hitting %s for %s",
                  t,
                  baseUrl,
                  requestId
                )
      ...
      LOG.info(
        String.format("Got exception %s when hitting %s for %s", e, baseUrl, requestId)
      );
      agentResponseDatastore.addAgentResponse(
        requestId,
        requestType,
  ...
  public AgentRequestsStatus getRequestsStatus(
    ...
    for (BaragonAgentMetadata agentMetadata : getAgents(
      request.getLoadBalancerService().getLoadBalancerGroups()
    )) {
      final String baseUrl = agentMetadata.getBaseAgentUri();

      Optional<Long> maybePendingRequestTime = agentResponseDatastore.getPendingRequest(
        request.getLoadBalancerRequestId(),
        baseUrl
      );
      if (maybePendingRequestTime.isPresent()) {
        if (
          (System.currentTimeMillis() - maybePendingRequestTime.get()) >
          baragonAgentRequestTimeout
        ) {
          LOG.info(
            "Request {} reached maximum pending request time",
            request.getLoadBalancerRequestId()
          );
          agentResponseDatastore.setPendingRequestStatus(
            request.getLoadBalancerRequestId(),
      ...
      final Optional<AgentResponseId> maybeAgentResponseId = agentResponseDatastore.getLastAgentResponseId(
        request.getLoadBalancerRequestId(),
        requestType,
        baseUrl
      );
      ...
      Optional<AgentResponse> maybeLastResponse = agentResponseDatastore.getAgentResponse(
        request.getLoadBalancerRequestId(),
        requestType,
        maybeAgentResponseId.get(),
        baseUrl
      );
      ...
      if (!missingTemplate) {
        ...
        if (
          (agentResponseId.getAttempt() < baragonAgentMaxAttempts - 1) &&
          !agentResponseId.isSuccess()
        ) {
          return AgentRequestsStatus.RETRY;
        } else {
    ...
    if (!missingTemplateExceptions.isEmpty() && allTrue(missingTemplateExceptions)) {
      ...
      return action.equals(RequestAction.RELOAD)
        ? AgentRequestsStatus.INVALID_REQUEST_NOOP
        : AgentRequestsStatus.FAILURE;
    }
  }
  ...
  public boolean invalidAgentCount(String loadBalancerGroup) {
    ...
    int targetCount = loadBalancerDatastore
      .getTargetCount(loadBalancerGroup)
      .or(configuration.getDefaultTargetAgentCount());
    return (
      agentCount == 0 ||
      (configuration.isEnforceTargetAgentCount() && agentCount < targetCount)
    );
  }

  public boolean hasMissingTemplate(Optional<AgentResponse> maybeLastResponse) {
    return (
      maybeLastResponse.isPresent() &&
      maybeLastResponse.get().getContent().isPresent() &&
      maybeLastResponse.get().getContent().get().contains("MissingTemplateException")
    );
  }
