...
public class ApplicationLoadBalancer extends ElasticLoadBalancer {
  ...
  private static final String DEREGISTRATION_DELAY_ATTR =
  ...
  public ApplicationLoadBalancer(
    ...
    @Named(
      BaragonServiceModule.BARAGON_AWS_ELB_CLIENT_V2
  ...
  @Override
  public AgentCheckInResponse removeInstance(
    Instance instance,
    String id,
  ...
  public AgentCheckInResponse removeInstance(String id, String trafficSourceName) {
    ...
    if (maybeTargetGroup.isPresent()) {
      ...
      TargetDescription targetDescription = new TargetDescription().withId(id);
      if (targetsOn(targetGroup).contains(targetDescription.getId())) {
        ...
        try {
          ...
          LOG.warn(
            "Failed to de-register instance {} from target group {}",
            id,
            targetGroup
          );
        }
  ...
  private AgentCheckInResponse getShutdownResponse(
      ...
      try {
        ...
        for (TargetGroupAttribute attribute : result.getAttributes()) {
          if (attribute.getKey().equals(DEREGISTRATION_DELAY_ATTR)) {
            LOG.info(
              "Target group {} has connection drain time of {}s",
              targetGroupArn,
              attribute.getValue()
            );
            maybeDrainTime =
              Optional.of(
                TimeUnit.SECONDS.toMillis(Long.parseLong(attribute.getValue()))
              );
          }
        }
      } catch (Exception e) {
    ...
    try {
      ...
      if (!healthResult.getTargetHealthDescriptions().isEmpty()) {
        switch (
          healthResult.getTargetHealthDescriptions().get(0).getTargetHealth().getState()
        ) {
          case "initial":
          case "healthy":
          case "unhealthy":
  ...
  @Override
  public AgentCheckInResponse checkRemovedInstance(
    String id,
    String trafficSourceName,
    String agentId
  ) {
    ...
    if (maybeTargetGroup.isPresent()) {
      return getShutdownResponse(
        maybeTargetGroup.get().getTargetGroupArn(),
  ...
  @Override
  public AgentCheckInResponse registerInstance(
    Instance instance,
    String id,
    String trafficSourceName,
    ...
    if (!maybeTargetGroup.isPresent()) {
      String message = String.format(
        "Target group with name %s not found",
        trafficSourceName
      );
      LOG.debug(message);
      return new AgentCheckInResponse(TrafficSourceState.ERROR, Optional.of(message), 0L);
  ...
  public AgentCheckInResponse registerInstance(String id, String targetGroup) {
    ...
    if (maybeTargetGroup.isPresent()) {
      ...
      return new AgentCheckInResponse(
        TrafficSourceState.ERROR,
        Optional.of("Target group not found"),
        0L
      );
    }
  }
  ...
  private AgentCheckInResponse registerInstance(String id, TargetGroup targetGroup) {
    TargetDescription newTarget = new TargetDescription().withId(id);
    ...
    if (!targetsOn(targetGroup).contains(newTarget.getId())) {
      try {
        ...
        LOG.info(
          "Registered instance {} with target group {}",
          newTarget.getId(),
          targetGroup
        );
      } catch (AmazonServiceException exn) {
  ...
  private AgentCheckInResponse instanceHealthResponse(
    TargetDescription targetDescription,
    ...
    try {
      ...
      if (!healthResult.getTargetHealthDescriptions().isEmpty()) {
        String targetState = healthResult
          .getTargetHealthDescriptions()
          .get(0)
          .getTargetHealth()
          .getState();
        switch (targetState) {
          ...
          default:
            String message = String.format(
              "Expected agent to be added but was in state %s",
              targetState
            );
    ...
    return new AgentCheckInResponse(
      ...
      exception,
      state == TrafficSourceState.PENDING
        ? configuration.get().getDefaultCheckInWaitTimeMs()
        : 0L
    );
  }

  @Override
  public AgentCheckInResponse checkRegisteredInstance(
    Instance instance,
    String id,
    ...
    if (maybeTargetGroup.isPresent()) {
      return instanceHealthResponse(
        ...
        .withId(
            trafficSource.getRegisterBy() == RegisterBy.INSTANCE_ID
  ...
  public void syncAll(Collection<BaragonGroup> baragonGroups) {
    ...
    Map<BaragonGroup, Map<TrafficSource, TargetGroup>> baragonGroupToTrafficSource = getTrafficSources(
      baragonGroups
    );
    for (Map.Entry<BaragonGroup, Map<TrafficSource, TargetGroup>> entry : baragonGroupToTrafficSource.entrySet()) {
      ...
      Collection<LoadBalancer> elbsForBaragonGroup = getLoadBalancersByBaragonGroup(
        allLoadBalancers,
        baragonGroup
      );
      Collection<BaragonAgentMetadata> baragonAgents = getAgentsByBaragonGroup(
        baragonGroup
      );
      ...
      for (Entry<TrafficSource, TargetGroup> targetGroupEntry : targetGroups.entrySet()) {
        try {
          ...
          LOG.debug(
            "Registering new instances for target group {}",
            trafficSource.getName()
          );
          guaranteeRegistered(
            trafficSource,
          ...
          if (configuration.isPresent() && configuration.get().isDeregisterEnabled()) {
            LOG.debug(
              "De-registering old instances for target group {}",
              trafficSource.getName()
            );
            deregisterRemovableTargets(
              trafficSource,
              baragonGroup,
              targetGroup,
              baragonAgents,
          ...
          exceptionNotifier.notify(
            exn,
            ImmutableMap.of("baragonGroup", baragonGroup.toString())
          );
        } catch (Exception exn) {
          LOG.error("Could not process ELB sync", exn);
          exceptionNotifier.notify(
            exn,
            ImmutableMap.of("groups", baragonGroup.toString())
          );
        }
      }
    }
  ...
  private Map<BaragonGroup, Map<TrafficSource, TargetGroup>> getTrafficSources(
    Collection<BaragonGroup> baragonGroups
  ) {
    ...
    for (BaragonGroup baragonGroup : baragonGroups) {
      Set<TrafficSource> albSources = baragonGroup
        ...
        .stream()
        .filter(
          trafficSource -> trafficSource.getType() == TrafficSourceType.ALB_TARGET_GROUP
        )
    ...
    Map<String, TargetGroup> namedTargetGroups = targetGroups
      .stream()
    ...
    for (BaragonGroup baragonGroup : trafficSources.keySet()) {
      ...
      for (TrafficSource trafficSource : trafficSources.get(baragonGroup)) {
        targetGroupsInBaragonGroup.put(
          trafficSource,
          namedTargetGroups.get(trafficSource.getName())
        );
      }
  ...
  public Collection<LoadBalancer> getAllLoadBalancers() {
    ...
    while (!Strings.isNullOrEmpty(nextPage)) {
      loadBalancersRequest = new DescribeLoadBalancersRequest().withMarker(nextPage);
  ...
  public Collection<Listener> getListenersForElb(String elbName) {
    ...
    if (maybeLoadBalancer.isPresent()) {
      ...
      while (!Strings.isNullOrEmpty(nextMarker)) {
        listenersRequest = new DescribeListenersRequest().withMarker(nextMarker);
  ...
  public Collection<Rule> getRulesByListener(String listenerArn) {
    ...
    return elbClient.describeRules(rulesRequest).getRules();
  }

  public Listener createListener(CreateListenerRequest createListenerRequest) {
    return elbClient.createListener(createListenerRequest).getListeners().get(0);
  }

  public Listener modifyListener(ModifyListenerRequest modifyListenerRequest) {
    return elbClient.modifyListener(modifyListenerRequest).getListeners().get(0);
  }
  ...
  public Rule createRule(CreateRuleRequest createRuleRequest) {
    return elbClient.createRule(createRuleRequest).getRules().get(0);
  }

  public Rule modifyRule(ModifyRuleRequest modifyRuleRequest) {
    return elbClient.modifyRule(modifyRuleRequest).getRules().get(0);
  }

  public void deleteRule(String ruleArn) {
    DeleteRuleRequest deleteRuleRequest = new DeleteRuleRequest().withRuleArn(ruleArn);

    elbClient.deleteRule(deleteRuleRequest);
  }

  public TargetGroup createTargetGroup(CreateTargetGroupRequest targetGroupRequest) {
    return elbClient.createTargetGroup(targetGroupRequest).getTargetGroups().get(0);
  }

  public TargetGroup modifyTargetGroup(
    ModifyTargetGroupRequest modifyTargetGroupRequest
  ) {
    return elbClient.modifyTargetGroup(modifyTargetGroupRequest).getTargetGroups().get(0);
  }
  ...
  private Collection<LoadBalancer> getLoadBalancersByBaragonGroup(
   ...
   */
  private void guaranteeRegistered(
    TrafficSource trafficSource,
    TargetGroup targetGroup,
   ...
   */
  private void deregisterRemovableTargets(
    TrafficSource trafficSource,
    BaragonGroup baragonGroup,
    TargetGroup targetGroup,
    ...
    Collection<TargetDescription> removableTargets = listRemovableTargets(
      trafficSource,
      baragonGroup,
      targets,
      agents
    ...
    LOG.info(
      "removableTargets.size()={}, baragonGroup.minHealthyAgents={}",
      removableTargets.size(),
      baragonGroup.getMinHealthyAgents()
    );

    Map<TargetDescription, TargetHealthDescription> targetToHealthDescriptionMap = getTargetToHealthDescriptionMap(
      targetGroup
    );
    ...
    for (TargetDescription removableTarget : removableTargets) {
      ...
      try {
        ...
        boolean shouldNotDeRegisterTarget =
          configuration.isPresent() &&
          !configuration.get().isRemoveLastHealthyEnabled() &&
          isPartOfMinHealthyAgents(
            baragonGroup,
            removableTarget,
            targetToHealthDescriptionMap
          );
        if (shouldNotDeRegisterTarget) {
          LOG.info(
            "Will not de-register target {} because configuration.get().isRemoveLastHealthyEnabled()==false isPartOfMinHealthyAgents=true in {}",
            removableTarget,
            targetGroup
          );
        } else {
          ...
          elbClient.deregisterTargets(
            new DeregisterTargetsRequest()
              .withTargetGroupArn(targetGroup.getTargetGroupArn())
              .withTargets(removableTarget)
          );
          LOG.info(
            "De-registered target {} from target group {}",
            removableTarget,
            targetGroup
          );
        ...
        LOG.error(
          "Could not de-register target {} from target group {} due to error",
          removableTarget,
          targetGroup,
          acexn
        );
        exceptionNotifier.notify(
          acexn,
          ImmutableMap.of("targetGroup", targetGroup.getTargetGroupName())
        );
      }
    }
  }

  private Map<TargetDescription, TargetHealthDescription> getTargetToHealthDescriptionMap(
    TargetGroup targetGroup
  ) {
    return elbClient
  ...
  private void guaranteeSameVPC(
    ...
    for (BaragonAgentMetadata agent : agents) {
      if (agent.getEc2().getVpcId().isPresent()) {
        if (!agent.getEc2().getVpcId().get().equals(vpcId)) {
          LOG.error(
            "Agent {} not on same VPC as its target group {}",
            agent,
            targetGroup
          );
          throw new IllegalStateException(
            String.format(
              "Agent %s not on same VPC as its target group %s",
              agent,
              targetGroup
            )
          );
        ...
        throw new IllegalStateException(
          String.format("Agent %s not assigned to a VPC", agent)
        );
      }
    }
    ...
    for (LoadBalancer loadBalancer : loadBalancers) {
      if (!vpcId.equals(loadBalancer.getVpcId())) {
        LOG.error(
          "Load balancer {} on different VPC from target group {}",
          loadBalancer,
          targetGroup
        );
        throw new IllegalStateException(
          String.format(
            "Load balancer %s on different VPC from target group %s",
            loadBalancer,
            targetGroup
          )
        );
  ...
  private void guaranteeAzEnabled(
    Collection<BaragonAgentMetadata> agents,
    Collection<LoadBalancer> loadBalancers
  ) {
    for (LoadBalancer loadBalancer : loadBalancers) {
      ...
      for (BaragonAgentMetadata agent : agents) {
        if (
          agent.getEc2().getAvailabilityZone().isPresent() &&
          !azNames.contains(agent.getEc2().getAvailabilityZone().get())
        ) {
          guaranteeHasAllSubnets(agent.getEc2().getSubnetId().asSet(), loadBalancer);
        }
      }
    }
  }
   ...
   */
  private boolean isPartOfMinHealthyAgents(
    BaragonGroup group,
    TargetDescription target,
    Map<TargetDescription, TargetHealthDescription> getTargetToHealthDescriptionMap
  ) {
  ...
  private void guaranteeHasAllSubnets(
    ...
    try {
      ...
      LOG.error(
        "Could not attach subnets {} to load balancer {} due to error",
        subnetsToAdd,
  ...
  private void guaranteeHasAllTargets(
    ...
    for (BaragonAgentMetadata agent : baragonAgents) {
      try {
        if (
          ...
          (
            trafficSource.getRegisterBy() == RegisterBy.PRIVATE_IP &&
            agent.getEc2().getPrivateIp().isPresent()
          )
        ) {
          String id = trafficSource.getRegisterBy() == RegisterBy.INSTANCE_ID
            ? agent.getEc2().getInstanceId().get()
            : agent.getEc2().getPrivateIp().get();
          if (agentShouldRegisterInTargetGroup(id, targets)) {
            targetDescriptions.add(new TargetDescription().withId(id));
        ...
        LOG.error(
          "Could not create request to register agent {} due to error",
          agent,
          exn
        );
    ...
    if (targetDescriptions.isEmpty()) {
      ...
      try {
        ...
        elbClient.registerTargets(registerTargetsRequest);
        LOG.info(
          "Registered targets {} onto target group {}",
          targetDescriptions,
          targetGroup
        );
      } catch (AmazonClientException acexn) {
        LOG.error(
          "Failed to register targets {} onto target group {}",
          targetDescriptions,
          targetGroup
        );
        exceptionNotifier.notify(
  ...
  private boolean agentShouldRegisterInTargetGroup(
    String id,
    Collection<TargetDescription> targets
  ) {
  ...
  private Collection<TargetDescription> listRemovableTargets(
    TrafficSource trafficSource,
    BaragonGroup baragonGroup,
    Collection<TargetDescription> targetsOnGroup,
    Collection<BaragonAgentMetadata> agentsInBaragonGroup
  ) {
    Collection<String> agentIds = trafficSource.getRegisterBy() == RegisterBy.INSTANCE_ID
      ? instanceIds(agentsInBaragonGroup)
    ...
    for (TargetDescription targetDescription : targetsOnGroup) {
      if (
        !agentIds.contains(targetDescription.getId()) &&
        canDeregisterAgent(baragonGroup, targetDescription.getId())
      ) {
  ...
  private Collection<BaragonAgentMetadata> getAgentsByBaragonGroup(
    BaragonGroup baragonGroup
  ) {
