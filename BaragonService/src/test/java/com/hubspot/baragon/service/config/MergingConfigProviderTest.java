...
public class MergingConfigProviderTest {
  ...
  private ConfigurationSourceProvider buildConfigSourceProvider(String baseFileName) {
    ...
    return new MergingConfigProvider(
        ...
        public InputStream open(String path) throws IOException {
          ...
          if (inputStream == null) {
            throw new FileNotFoundException(
              String.format("File %s not found in test resources directory", path)
            );
          } else {
            return inputStream;
          }
  ...
  public void itMergesNestedConfigsProperly() throws Exception {
    ...
    assertEquals(
      100,
      (int) mergedConfig.getZooKeeperConfiguration().getSessionTimeoutMillis()
    );
    assertEquals(
      "override-namespace",
      mergedConfig.getZooKeeperConfiguration().getZkNamespace()
    );
    assertEquals(
      100,
      (int) mergedConfig.getZooKeeperConfiguration().getConnectTimeoutMillis()
    );
    assertEquals(
      100,
      (int) mergedConfig.getZooKeeperConfiguration().getRetryBaseSleepTimeMilliseconds()
    );
  ...
  public void itThrowsExnOnNonObjectOverride() throws Exception {
    Assertions.assertThrows(
      ...
      () -> buildConfigSourceProvider(DEFAULT_PATH).open(JUST_A_STRING_PATH)
    );
  }
  ...
  public void itThrowsExnOnNonObjectDefault() throws Exception {
    Assertions.assertThrows(
      IllegalArgumentException.class,
      () -> buildConfigSourceProvider(JUST_A_STRING_PATH).open(OVERRIDE_PATH)
    );
  }
  ...
  public void itThrowsExnOnNotFoundOverridePath() throws Exception {
    Assertions.assertThrows(
      ...
      () -> buildConfigSourceProvider(DEFAULT_PATH).open(DOESNT_EXIST_PATH)
    );
  }
  ...
  public void itThrowsExnOnNotFoundDefaultPath() throws Exception {
    Assertions.assertThrows(
      FileNotFoundException.class,
      () -> buildConfigSourceProvider(DOESNT_EXIST_PATH).open(OVERRIDE_PATH)
    );
  }
}
