...
public class AgentRequestManager {
  ...
  public List<AgentBatchResponseItem> processRequests(
    ...
    Map<String, Long> numRequestsByService = batch
      .stream()
      .map(
        requestItem -> {
          ...
          if (maybeRequest.isPresent()) {
            services.computeIfAbsent(
              ...
              r -> getOldService(maybeRequest.get())
            );
          }
          return maybeRequest;
        }
      ...
      .collect(
        Collectors.groupingBy(
          r -> r.getLoadBalancerService().getServiceId(),
          Collectors.counting()
        )
      );
    ...
    Map<String, Collection<UpstreamInfo>> existingUpstreamsForThisBatch = services
      .keySet()
      .stream()
      .collect(
        Collectors.toMap(
          ...
          serviceId -> {
            try {
              ...
              throw new RuntimeException(
                "Unable to get upstream information for service {}",
                e
              );
            }
          }
    ...
    for (BaragonRequestBatchItem item : batch) {
      ...
      responses.add(
        getResponseItem(
          processRequest(
            item.getRequestId(),
            existingUpstreamsForThisBatch,
            services,
            requests,
            actionForBatchItem(item),
            !isLast,
            Optional.of(i)
          ),
          item
  ...
  private AgentBatchResponseItem getResponseItem(
    Response httpResponse,
    BaragonRequestBatchItem item
  ) {
    Optional<String> maybeMessage = httpResponse.getEntity() != null
      ? Optional.of(httpResponse.getEntity().toString())
      : Optional.<String>absent();
    return new AgentBatchResponseItem(
      item.getRequestId(),
  ...
  public Response processRequest(
    ...
    if (requests.get(requestId) == null || !requests.get(requestId).isPresent()) {
      return Response
        ...
        .entity(String.format("Request %s does not exist", requestId))
        .build();
    }
    ...
    return processRequest(
      ...
      action,
      request,
      services.get(request.getLoadBalancerService().getServiceId()),
      existingUpstreams,
      delayReload,
      batchItemNumber
    );
  ...
  public Response processRequest(
    ...
    try {
      ...
      switch (action) {
        ...
        case REVERT:
          ...
          return revert(
            request,
            maybeOldService,
            existingUpstreams.computeIfAbsent(serviceId, key -> new ArrayList<>()),
            delayReload,
            batchItemNumber
          );
        default:
          ...
          if (request.isPurgeCache()) {
            purgeCache(
              maybeOldService.or(request.getLoadBalancerService()).getServiceId()
            );
          }
          return response;
      }
      ...
      LOG.error(
        "Couldn't acquire agent lock for {} in {} ms",
        requestId,
        agentLockTimeoutMs,
        e
      );
      return Response
        ...
        .entity(
          String.format(
            "Couldn't acquire agent lock for %s in %s ms. Lock Info: %s",
            requestId,
            agentLockTimeoutMs,
            e.getLockInfo()
          )
      ...
      return Response
        ...
        .entity(
          String.format(
            "Caught exception while %sING for request %s: %s",
            action,
            requestId,
            e.getMessage()
          )
      ...
      LOG.info(
        "Done processing {} request {} after {}ms)",
        action,
        requestId,
  ...
  public Response purgeCache(String serviceId) {
    ...
    if (!maybeService.isPresent()) {
      return Response
        .status(Response.Status.INTERNAL_SERVER_ERROR)
        .entity(String.format("Could not find service with serviceId=%s", serviceId))
        .build();
    }
  ...
  private Response apply(
    BaragonRequest request,
    Optional<BaragonService> maybeOldService,
    Collection<UpstreamInfo> existingUpstreams,
    boolean delayReload,
    Optional<Integer> batchItemNumber
  )
    throws Exception {
    ...
    if (movedOffLoadBalancer(request)) {
      ...
      internalStateCache.put(
        update.getService().getServiceId(),
        new BasicServiceContext(update.getService(), update.getUpstreams())
      );
    }
    return Response.ok().build();
  }

  private Response revert(
    BaragonRequest request,
    Optional<BaragonService> maybeOldService,
    Collection<UpstreamInfo> existingUpstreams,
    boolean delayReload,
    ...
    try {
      configHelper.apply(
        update,
        Optional.<BaragonService>absent(),
        false,
        request.isNoReload(),
        request.isNoValidate(),
        delayReload,
  ...
  private ServiceContext getApplyContext(
    BaragonRequest request,
    Collection<UpstreamInfo> existingUpstreams
  )
    throws Exception {
  ...
  private void triggerTesting() throws Exception {
    if (
      maybeTestingConfiguration.isPresent() &&
      maybeTestingConfiguration.get().isEnabled() &&
      maybeTestingConfiguration.get().getApplyDelayMs() > 0
    ) {
      Thread.sleep(maybeTestingConfiguration.get().getApplyDelayMs());
    }

    if (
      maybeTestingConfiguration.isPresent() && maybeTestingConfiguration.get().isEnabled()
    ) {
