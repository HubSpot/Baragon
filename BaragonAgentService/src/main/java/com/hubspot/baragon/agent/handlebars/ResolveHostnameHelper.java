...
public class ResolveHostnameHelper implements Helper<Object> {
  ...
  public CharSequence apply(Object upstream, Options options) {
    if (upstream instanceof String) {
      ...
      final Optional<String> maybeResolvedUpstream = resolver.resolveUpstreamDNS(
      ...
      LOG.trace(
        ...
        upstreamInfo,
        upstreamInfo.getResolvedUpstream(),
        maybeResolvedUpstream,
        upstreamInfo.getUpstream()
      );
      String resolved = upstreamInfo
        .getResolvedUpstream()
        .or(maybeResolvedUpstream)
        .or(upstreamInfo.getUpstream());
      LOG.trace("Resolved {} to {}", upstreamInfo, resolved);
      return resolved;
    } else {
      LOG.error(
        "resolveHostname called with invalid context {} and options {}",
        upstream,
        options
      );
      throw new IllegalArgumentException(
        String.format("Don't know how to process upstream %s", upstream.toString())
      );
    }
  }
}
