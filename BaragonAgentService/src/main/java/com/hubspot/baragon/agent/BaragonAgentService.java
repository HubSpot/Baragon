...
public class BaragonAgentService extends Application<BaragonAgentConfiguration> {
  ...
  public void initialize(Bootstrap<BaragonAgentConfiguration> bootstrap) {
    guiceBundle =
      GuiceBundle
        .defaultBuilder(BaragonAgentConfiguration.class)
  ...
  @Override
  public void run(BaragonAgentConfiguration configuration, Environment environment)
    throws Exception {
    ...
    environment
      ...
      .addServerLifecycleListener(
        server ->
          guiceBundle
            .getInjector()
            .getInstance(LifecycleHelper.class)
            .checkStateNodeVersion()
      );
  }
