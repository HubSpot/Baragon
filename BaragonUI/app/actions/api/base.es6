...
export function buildApiAction(actionName, opts = {}, keyFunc = undefined) {
  ...
  function trigger(...args) {
    return (dispatch) => {
      ...
      let apiResponse;
      let authKey = '';
      if (localStorage.getItem('baragonAuthKey')) {
        if (options.url.includes('?')) {
          authKey = `&authkey=${localStorage.getItem('baragonAuthKey')}`
        } else {
          authKey = `?authkey=${localStorage.getItem('baragonAuthKey')}`
        }
      }
      return fetch(config.apiRoot + options.url + authKey, _.extend({credentials: 'include'}, _.omit(options, 'url')))
        .then(response => {
